...
public class ReservaController {
    ...
    private static final String SALA_NO_ENCONTRADA = "Sala no encontrada";
    ...
    public ResponseEntity<Object> buscarSalas(
    ...
    @GetMapping
    public ResponseEntity<Object> misReservas(
        ...
        var resultado = reservaRepository.findByClienteIdOrderByFechaDescHoraInicioDesc(clienteId, pageable)
    ...
    @Transactional
    @PostMapping
    public ResponseEntity<Object> crear(@Valid @RequestBody ReservaRequest req, HttpServletRequest request) {
        var session = request.getSession(false);
        if (session == null || session.getAttribute(AuthInterceptor.SESSION_USER_ID) == null) return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        Long clienteId = (Long) session.getAttribute(AuthInterceptor.SESSION_USER_ID);

        var maybeSala = salaRepository.findById(req.getSalaId());
        if (maybeSala.isEmpty()) return ResponseEntity.status(HttpStatus.NOT_FOUND).body(Map.of(ERROR,SALA_NO_ENCONTRADA));
        ...
        validarReglas(req.getHoraInicio(), req.getHoraFin());

        var candidatas = reservaRepository.findBySalaIdAndFechaAndEstadoNot(req.getSalaId(), req.getFecha(), EstadoReserva.CANCELADA);
    ...
    public ResponseEntity<Object> disponibilidad(@RequestParam(required = false) Long salaId,
                                           @RequestParam(required = false) LocalDate fecha,
                                           @RequestParam(required = false) LocalTime horaInicio,
                                           @RequestParam(required = false) LocalTime horaFin,
                                           HttpServletRequest request) {
    ...
    public ResponseEntity<Object> horarioDia(@RequestParam Long salaId, @RequestParam LocalDate fecha) {
        var maybeSala = salaRepository.findById(salaId);
        if (maybeSala.isEmpty()) return ResponseEntity.status(HttpStatus.NOT_FOUND).body(Map.of(ERROR, SALA_NO_ENCONTRADA));
    ...
    private void validarReglas(LocalTime horaInicio, LocalTime horaFin) {
    ...
    @Transactional
    @PutMapping("/{id}")
    public ResponseEntity<Object> editar(@PathVariable Long id, @Valid @RequestBody ReservaRequest req,
                                    HttpServletRequest request) {
        var session = request.getSession(false);
    ...
    public ResponseEntity<Object> cancelar(@PathVariable Long id, HttpServletRequest request) {
        var session = request.getSession(false);
        if (session == null || session.getAttribute(AuthInterceptor.SESSION_USER_ID) == null) return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        Long clienteId = (Long) session.getAttribute(AuthInterceptor.SESSION_USER_ID);
        var maybeCliente = usuarioRepository.findById(clienteId);
